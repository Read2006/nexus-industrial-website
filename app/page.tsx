...
const heroSlides = [
  '/new-heroes/oil-gas.jpeg',
  '/new-heroes/cement.jpeg',
  '/new-heroes/mechanical.jpeg',
  '/new-heroes/steel.jpeg',
  '/new-heroes/power.jpeg',
  '/new-heroes/textile.jpeg',
  '/new-heroes/fertilizer.jpeg',
]
...
const industries = [
  { name: 'Oil & Gas', href: '/oil-gas', img: '/new-heroes/oil-gas.jpeg' },
  { name: 'Cement', href: '/cement', img: '/new-heroes/cement.jpeg' },
  { name: 'Textile', href: '/textile', img: '/new-heroes/textile.jpeg' },
  { name: 'Steel/Aluminium', href: '/steel', img: '/new-heroes/steel.jpeg' },
  { name: 'Fertilizer', href: '/fertilizer', img: '/new-heroes/fertilizer.jpeg' },
  { name: 'Aviation', href: '/aviation', img: '/new-images/avaition.jfif' },
  { name: 'Power', href: '/power', img: '/new-heroes/power.jpeg' },
  { name: 'FMCG', href: '/fmcg', img: '/industries/fmcg hero.jfif' },
]

const products = [
  { name: 'Mechanical Products', desc: 'Bearings, Pumps, Gearboxes, Belts & more', href: '/products/mechanical', img: '/new-heroes/mechanical.jpeg', badge: 'Bearings · Pumps · Seals' },
  ...
  { name: 'General Orders', desc: 'Custom sourcing for any industrial product or equipment', href: '/products/general', img: '/general-orders/hero.jpg', badge: 'Safety · Tools · Hardware' },
]
...
const whyChooseUs = [
  ...
  { icon: '📋', title: 'Procurement Support' },
  { icon: '🤝', title: 'Trusted Supplier Network' },
]
...
export default function HomePage() {
  ...
  useEffect(() => {
    const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    if (reduce) return
    const id = setInterval(() => setAboutSlide((s) => (s + 1) % aboutSlides.length), 4500)
    return () => clearInterval(id)
  }, [])
  ...
  return (
    <div className="min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      >
        ...
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(7,26,22,0.82) 0%, rgba(13,43,36,0.72) 45%, rgba(15,61,50,0.62) 100%)' }}></div>
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-transparent via-teal-400/80 to-transparent"></div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-24 relative z-10 flex items-center min-h-[70vh] md:min-h-[85vh]">
          <div className="max-w-3xl space-y-6">
            <span className="inline-flex items-center gap-2 bg-teal-400/10 border border-teal-400/30 text-teal-300 px-4 py-2 rounded-full text-xs font-bold tracking-widest uppercase min-h-[44px]">
              <span className="w-1.5 h-1.5 rounded-full bg-teal-400 animate-pulse"></span>
              Industrial Supply & Sourcing — Pakistan
            </span>
            <h1 className="text-white font-black leading-[0.95] tracking-tighter text-3xl sm:text-4xl md:text-5xl lg:text-6xl uppercase">
              Your Trusted<br />
              <span className="text-teal-400">Industrial</span><br />
              Supply Partner
            </h1>
            <p className="text-teal-50/80 text-base md:text-lg leading-relaxed max-w-lg font-medium">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {['Genuine Parts', '24hr Quote', 'Global Sourcing'].map(item => (
                <span key={item} className="flex items-center gap-1.5 text-white/70 text-xs font-bold uppercase tracking-wide">{item}</span>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 pt-2">
              <a href="/contact" className="inline-flex items-center justify-center gap-2 rounded-lg px-8 py-4 min-h-[44px] font-black text-sm tracking-widest uppercase bg-gradient-to-r from-teal-500 to-teal-700 text-white shadow-xl shadow-teal-500/30 hover:shadow-teal-500/60 hover:-translate-y-0.5 transition-all">
                Get a Quote →
              </a>
              <a href="/#products" className="inline-flex items-center justify-center gap-2 rounded-lg border-2 border-teal-400/50 text-teal-300 px-8 py-4 min-h-[44px] font-black text-sm tracking-widest uppercase hover:bg-teal-400/10 transition-all">
                Browse Products
              </a>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 max-w-xl">
              {[
                ...
                <div key={s.label} className="bg-black/40 backdrop-blur-sm border border-white/15 rounded-xl p-3">
                  <p className="text-teal-400 font-black text-xl leading-none">{s.val}</p>
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex gap-2">
          {heroSlides.map((_, i) => (
            <button
              ...
              onClick={() => setSlide(i)}
              className={`h-2.5 rounded-full transition-all ${i === slide ? 'w-8 bg-teal-400' : 'w-2.5 bg-white/40'}`}
            />
          ))}
        </div>
      ...
      {/* ABOUT */}
      <section id="about-section" className="py-16 md:py-24 bg-white scroll-mt-20 overflow-x-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div className="space-y-6">
              ...
              <p className="text-gray-600 text-base leading-relaxed">
                At Nexus Industrial Solutions, we provide reliable industrial procurement, sourcing, and supply solutions to businesses across multiple industries. With a strong global supplier network and industry expertise, we deliver high-quality products that meet our clients&apos; technical and commercial requirements.
              </p>
              <p className="text-gray-600 text-base leading-relaxed">
                Our services include industrial procurement, global sourcing, import &amp; export, supply chain support, and customized product solutions. We work with trusted suppliers to ensure competitive pricing, consistent quality, and timely delivery.
              </p>
              <p className="text-gray-600 text-base leading-relaxed">
                We serve various industries including Manufacturing, Construction, Engineering, Power, FMCG, Oil &amp; Gas, Infrastructure, and General Trading. Our focus is to build long-term partnerships through reliable solutions, transparent communication, and exceptional service.
              </p>
              <div className="flex flex-wrap gap-4">
                <a href="/contact" className="inline-flex items-center justify-center gap-2 bg-teal-700 hover:bg-teal-800 text-white px-6 py-3 min-h-[44px] rounded-lg font-black text-sm uppercase tracking-wide transition">Get In Touch →</a>
                <a href="/#products" className="inline-flex items-center justify-center gap-2 border-2 border-teal-700 text-teal-700 hover:bg-teal-50 px-6 py-3 min-h-[44px] rounded-lg font-black text-sm uppercase tracking-wide transition">Our Products</a>
              </div>
            </div>

            <div className="relative rounded-2xl overflow-hidden shadow-2xl h-[320px] sm:h-[420px] lg:h-[520px]">
              {aboutSlides.map((src, i) => (
                <img
                  ...
                  style={{ opacity: i === aboutSlide ? 1 : 0 }}
                />
              ))}
          ...
          <div className="mt-16">
            <h3 className="text-gray-900 text-2xl font-black tracking-tight uppercase mb-8 text-center">Why Choose Us</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {whyChooseUs.map(item => (
                <div key={item.title} className="border border-teal-100 rounded-xl p-5 bg-teal-50/40 text-center">
                  <p className="text-2xl mb-2">{item.icon}</p>
                  <p className="text-gray-800 font-bold text-sm leading-snug">{item.title}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
            {[
              ...
              { val: '500+', label: 'Products Sourced' },
              { val: '24-Hour', label: 'Quote Turnaround' },
              { val: '9+', label: 'Industries Served' },
            ].map(s => (
              <div key={s.label} className="border border-teal-100 rounded-xl p-5 bg-white text-center">
          ...
          <div className="mt-10 bg-teal-50 border border-teal-100 rounded-xl p-5 md:p-6">
            <p className="text-teal-900 text-sm leading-relaxed">
              <span className="font-black">Compliance Note:</span> Nexus Industrial Solutions is a General Order Supplier and Procurement Contractor. We source products according to client specifications from reputable local and international suppliers. Unless specifically authorized in writing, we do not represent ourselves as an authorized distributor, dealer, or agent of any manufacturer.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* PRODUCTS */}
      <section id="products" className="py-16 md:py-24 bg-gray-50 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
            ...
            <a href="/brands" className="inline-flex items-center justify-center gap-2 border-2 border-teal-700 text-teal-700 hover:bg-teal-700 hover:text-white font-black text-xs tracking-widest px-6 py-3 min-h-[44px] rounded-lg transition-all uppercase shrink-0">View All Brands →</a>
          </div>
      ...
      {/* HOW IT WORKS */}
      <section className="py-16 md:py-24" style={{ background: 'linear-gradient(135deg, #071a16 0%, #0d2b24 50%, #0f3d32 100%)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
      ...
      {/* INDUSTRIES */}
      <section id="industries" className="py-16 md:py-24 bg-gradient-to-br from-teal-50/50 via-white to-white scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
      ...
      {/* BRANDS */}
      <section id="brands-section" className="py-16 md:py-24 bg-gray-50 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-14">
            <span className="text-teal-700 font-black tracking-[0.4em] text-[11px] uppercase mb-3 block">BRANDS WE SERVE</span>
            <h2 className="text-gray-900 text-3xl lg:text-4xl font-black tracking-tighter uppercase">Brands We <span className="text-teal-700">Serve</span></h2>
            <p className="text-gray-500 max-w-2xl mx-auto mt-4">Connecting businesses with trusted global suppliers and 190+ leading brands.</p>
          </div>
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
            ...
            <a href="/brands" className="bg-gradient-to-br from-teal-700 to-teal-900 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:from-teal-600 hover:to-teal-800 transition-all min-h-[90px]">
              <span className="text-teal-300 font-black text-lg">190+</span>
      ...
      <section id="contact-section" className="py-20 scroll-mt-20" style={{ background: 'linear-gradient(135deg, #071a16 0%, #0d2b24 50%, #0f3d32 100%)' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-4 justify-center">
            <a href="/contact" className="inline-flex items-center justify-center gap-2 bg-teal-500 hover:bg-teal-400 text-white px-10 py-4 min-h-[44px] rounded-lg font-black text-sm tracking-[0.2em] uppercase shadow-lg shadow-teal-500/20 hover:-translate-y-1 transition-all">
              CONTACT US NOW →
            </a>
            <a href="tel:03351503555" className="inline-flex items-center justify-center gap-2 border border-white/20 text-white/80 px-10 py-4 min-h-[44px] rounded-lg font-black text-sm tracking-[0.2em] uppercase hover:bg-white/5 hover:border-white/40 transition-all">
              CALL US
            </a>
          </div>
        </div>
